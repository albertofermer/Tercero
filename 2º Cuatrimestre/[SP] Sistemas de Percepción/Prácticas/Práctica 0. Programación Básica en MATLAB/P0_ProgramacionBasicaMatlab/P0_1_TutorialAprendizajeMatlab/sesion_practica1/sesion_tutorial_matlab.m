...
[Vordenado, posVordenado] = sort(V,'descend')
...
% aparece en A cada uno de sus posibles valores
for i = 1:size(valoresA,2)
    conteoValores(i) = sum(sum(A == valoresA(i)))
    %valores = A == valoresA(i);
    %conteoValores(i) = sum(valores(:))
end
% e) Representa con stem, valoresA frente a conteoValores

stem(valoresA,conteoValores,'filled','Color',[0 1 0])
grid on;
...
funcion_compara_matrices(M1,M3);
funcion_compara_matrices(M1,M4);