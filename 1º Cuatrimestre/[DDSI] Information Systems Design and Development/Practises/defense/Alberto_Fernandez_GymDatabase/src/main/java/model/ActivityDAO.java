...
public class ActivityDAO {
    ...
    /**
     * Adds a value to all the acitvities' price.
     *
     * @param value integer that you want to add to the prices.
     */
    public void addToAll(int value) {

        ArrayList<Activity> alist = getActList();
        Transaction tr = s.beginTransaction();
        try {
            for (int i = 0; i < alist.size(); i++) {
                Activity a = s.get(Activity.class, alist.get(i).getAId());
                a.setAPrize(alist.get(i).getAPrize() + value);
                s.update(a);
            }
