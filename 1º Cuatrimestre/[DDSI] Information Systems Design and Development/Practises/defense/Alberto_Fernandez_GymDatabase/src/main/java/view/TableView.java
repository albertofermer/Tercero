...
public class TableView {
    ...
    /**
     * Creates the model of the price's table.
     */
    public DefaultTableModel modelTablePrices = new DefaultTableModel() {
        @Override
        public boolean isCellEditable(int row, int column) {
            return false;
        }
    ...
    /**
     * Shows all the activities with its price.
     *
     * @param vPrice
     */
    public void setupTablePrices(UpdatePriceView vPrice) {

        vPrice.PriceTable.setModel(modelTablePrices);
        String[] columNames = {"Code", "Name", "Price"};
        modelTablePrices.setColumnIdentifiers(columNames);

        vPrice.PriceTable.getTableHeader().setResizingAllowed(true);
        vPrice.PriceTable.setAutoResizeMode(JTable.AUTO_RESIZE_LAST_COLUMN);

        vPrice.PriceTable.getColumnModel().getColumn(0).setPreferredWidth(50);
        vPrice.PriceTable.getColumnModel().getColumn(1).setPreferredWidth(50);
        vPrice.PriceTable.getColumnModel().getColumn(2).setPreferredWidth(50);
    }
     ...
     * @param a
     */
    public void fillTablePrices(ArrayList<Activity> a) {
        Object[] row = new Object[3];
        int numRows = a.size();
        cleanTablePrices();

        for (int i = 0; i < numRows; i++) {
            row[0] = a.get(i).getAId();
            row[1] = a.get(i).getAName();
            row[2] = a.get(i).getAPrize();
            modelTablePrices.addRow(row);
        }
    }
    ...
    private void cleanTablePrices() {
        for (int i = modelTablePrices.getRowCount() - 1; i >= 0; i--) {
            modelTablePrices.removeRow(i);
        }
    }
