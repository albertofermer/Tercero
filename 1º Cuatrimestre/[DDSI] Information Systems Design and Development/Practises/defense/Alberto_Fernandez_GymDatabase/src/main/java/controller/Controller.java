...
import view.MembershipFeeView;
import view.MessageView;
import view.RegisterView;
import view.TableView;
import view.UpdatePriceView;
...
public class Controller implements ActionListener {
    ...
    private UpdatePriceView vPrice = null;
    private MembershipFeeView vFee = null;
    ...
    public Controller() {
        ...
        vPrice = new UpdatePriceView();
        vFee = new MembershipFeeView();
    ...
    private void addListener() {
        ...
        mainwindow.UpdatePriceMenu.addActionListener(this);
        mainwindow.MemberFeeMenu.addActionListener(this);
        vPrice.UpdateAllPricesButton.addActionListener(this);
        vFee.SelectButton.addActionListener(this);
    ...
    public void actionPerformed(ActionEvent e) {
        switch (e.getActionCommand()) {
            case "Update Price":
                ...
                try {
                    ActivityDAO adao = new ActivityDAO(s);
                    vTable.setupTablePrices(vPrice);
                    vTable.fillTablePrices(adao.getActList());
                } catch (Exception ex) {
            ...
            case "Update All Prices":
                try {
                ActivityDAO adao = new ActivityDAO(s);
                adao.addToAll(Integer.parseInt(vPrice.addpriceValue.getText()));
                vTable.fillTablePrices(adao.getActList());
            } catch (NumberFormatException ex) {
                vMessage.Message("error", "The value must be an integer.");
            }
            break;

            case "Membership Fee":
                vFee.numberActivities.setText("0");
                vFee.MemberCode.setText("");
                vFee.membershipFee.setText("0");
                vFee.setVisible(true);
                break;
            case "Select":
             try {
                MemberDAO mdao = new MemberDAO(s);
                vFee.membershipFee.setText(String.valueOf(mdao.feeOfMember(vFee.MemberCode.getText())));
                vFee.numberActivities.setText(String.valueOf(mdao.getNumActivities(vFee.MemberCode.getText())));
            } catch (Exception ex) {
                if (ex.getMessage() != null) {
                    vMessage.Message("error", "Unknown Error:  " + ex.getMessage());
                } else {
                    vFee.numberActivities.setText("0");
                    vFee.MemberCode.setText("");
                    vFee.membershipFee.setText("0");
