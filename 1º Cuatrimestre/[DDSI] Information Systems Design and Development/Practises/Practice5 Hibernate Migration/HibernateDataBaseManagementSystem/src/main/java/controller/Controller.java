 ...
 */
    private void vTrainerTrainerTableMouseClicked() {
        ...
        for (Activity a : inCharge) {
            vTrainer.ActList.setSelectedValue(a.getAName(), true);
        }
        ...
        String[] f = values[7].split(" ");
        String[] t = f[0].split("-");
        System.out.println(Date.valueOf(f[0]));
        vTrainer.trainerDate.setDate(Date.valueOf(f[0]));
        ...
        switch (e.getActionCommand()) {
            ...
            case "trainerUpdate":
                ...
                for (int i = 0; i < vTrainer.ActList.getModel().getSize(); i++) {
                    if (vTrainer.ActList.isSelectedIndex(i)) {
                        activityList.add(adao.getActivityByName(vTrainer.ActList.getModel().getElementAt(i)));
                        //adao.getActivityByName(vTrainer.ActList.getModel().getElementAt(i)).setATrainerincharge(null);
                    }
                }
...
/**
 * Converts the date of the parameter with format: Fri Jul 05 00:00:00 CEST 1996
 * into the correct format for the application:  Jul 05 1996
 * @param date
 * @return 
 */
